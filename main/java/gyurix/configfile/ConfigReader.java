...
public class ConfigReader
{
  ...
  ConfigReader(int lvl, ConfigData key, ConfigData value) { this.blockLvl = lvl;
    this.key = key;
    this.value = value;
  }
  
  public ConfigData getData() { return this.keyRead ? this.key : this.value; }
  
  public void addComment(String com) {
    ConfigData data = getData();
    if (data.comment == null) {
      data.comment = com;
    } else { ConfigData tmp28_27 = data;tmp28_27.comment = (tmp28_27.comment + '\n' + com); } }
  
  final ConfigData value;
  final ConfigData key;
  public void handleInput(ArrayList<ConfigReader> readers, String line, int lvl) { ConfigData data = getData();
    ...
    if (line.startsWith("- ")) {
      ...
      if (lvl == this.blockLvl) {
        ...
        ((ConfigReader)readers.get(size)).getData().mapData.put(key, value);
      }
      else {
      ...
      if (s.length == 2) {
          ConfigData key;
        if (this.keyRead) {
          this.key.stringData += ConfigData.unescape(new StringBuilder().append("\n").append(s[0]).toString());
          key = this.key;
        }
        else {
        ...
        if (lvl == this.blockLvl) {
          int size = readers.size() - 2;
          readers.remove(size + 1);
          data = ((ConfigReader)readers.get(size)).getData();
          if (data.mapData == null) {
            data.mapData = new LinkedHashMap();
          }
          data.mapData.put(key, value);
      ...
      } else { data.stringData += ConfigData.unescape(new StringBuilder().append("\n").append(line).toString());
      }
    }
  }
}
